...
local function buildGUI(floors)
	...
	local existing = LocalPlayer.PlayerGui:FindFirstChild("PetBaseGUI")
...
-- ══════════════════════════════════════════
--  ПОСТРОЕНИЕ GUI И ЕГО ОБНОВЛЕНИЕ
-- ══════════════════════════════════════════

-- Функция, которая очищает старый список и рисует только новые слоты/петов
local function updatePetList(scrollFrame, floors, statusBar)
	-- Очищаем только карточки внутри скролл-фрейма
	...
	-- Считаем питомцев для статус-бара
	local totalPets = 0
	for _, f in ipairs(floors) do
		for _, s in ipairs(f.slots) do
			if s.pet then totalPets += 1 end
	...
	local statusLabel = statusBar:FindFirstChildOfClass("TextLabel")
	if statusLabel then
		statusLabel.Text = string.format("📊  %d Floors  •  %d pets found", #floors, totalPets)
	end

	-- Пересоздаем карточки этажей и петов
	for fi, floorData in ipairs(floors) do
		local floorHeader = Instance.new("Frame")
		floorHeader.Size = UDim2.new(1, 0, 0, 32)
		floorHeader.BackgroundColor3 = Color3.fromRGB(30, 38, 68)
...
-- Функция создает ТОЛЬКО внешнюю коробку (рамку) один единственный раз
local function buildGUI(initialFloors)
	local existing = LocalPlayer.PlayerGui:FindFirstChild("PetBaseGUI")
	if existing then existing:Destroy() end
	...
	-- Первичное заполнение контента
	updatePetList(scrollFrame, initialFloors, statusBar)
	...
	minimizeBtn.MouseButton1Click:Connect(function()
		if not minimized then
			TweenService:Create(main, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {Size = UDim2.new(0, MAIN_W, 0, 52)}):Play()
			minimized = true
		else
			TweenService:Create(main, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {Size = UDim2.new(0, MAIN_W, 0, MAIN_H)}):Play()
			minimized = false
		end
	end)

	-- Логика Драг-н-Дропа (перетаскивание)
	local dragging, dragStart, startPos
	header.InputBegan:Connect(function(inp)
...
-- ══════════════════════════════════════════
--  ТОЧКА ВХОДА (ОБНОВЛЕНИЕ ТОЛЬКО СОДЕРЖИМОГО)
-- ══════════════════════════════════════════
...
task.spawn(function()
	...
	-- 1. Собираем базу и строим GUI каркас ОДИН раз
	local initialFloors = collectBaseData(plot)
	local screenGui = buildGUI(initialFloors)
	
	-- Достаем ссылки на список и статус бар, чтобы обновлять только их
	local scrollFrame = screenGui.Main.ScrollFrame
	local statusBar = screenGui.Main.StatusBar

	-- 2. Запускаем бесконечный цикл обновления контента в фоне
	while true do
		task.wait(5) -- Ждем 5 секунд
		
		-- Если GUI удалили или закрыли, останавливаем цикл
		if not screenGui or not screenGui.Parent then 
			break 
		end
		
		-- Сканируем базу на новые данные
		local updatedFloors = collectBaseData(plot)
		
		-- Обновляем ИСКЛЮЧИТЕЛЬНО карточки петов
		updatePetList(scrollFrame, updatedFloors, statusBar)
	end
end)
