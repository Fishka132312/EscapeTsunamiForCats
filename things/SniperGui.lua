...
local function createPetButton(petName)
    ...
    petImage.Position = UDim2.new(0, 10, 0.5, -11)
    ...
    -- =======================================================================
    -- БЕЗОПАСНАЯ И ЛЕГКАЯ ПОДСВЕТКА (БЕЗ ИЗМЕНЕНИЯ РАЗМЕРОВ)
    -- =======================================================================
    btn.MouseEnter:Connect(function()
        -- Если кнопка не выбрана, делаем фон чуточку светлее
    ...
    btn.MouseLeave:Connect(function()
        -- При уходе мыши возвращаем оригинальный цвет в зависимости от галочки
        updateCheck()
    end)
...
local function refreshPetList(filter)
    ...
    -- Родной и стабильный расчет размера скролла
